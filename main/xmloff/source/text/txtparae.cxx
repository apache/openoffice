    ...
    if ( rNextInfo.GetLevel() > 0 )
    {
        ...
        if ( nListLevelsToBeOpened > 0 )
        {
            ...
            do {
                ...
                // --> OD 2008-11-26 #158694#
                if ( bContinuingPreviousSubList )
                {
                    GetExport().AddAttribute( XML_NAMESPACE_TEXT,
                                              XML_CONTINUE_NUMBERING, XML_TRUE );
                    bContinuingPreviousSubList = false;
                }
                // <--
...
//		rPropSetHelper.getValues( xPropSet );
	...
	if( bHasContentEnum )
	{
		...
		// if hard attributes are existing. Therefor, it seems to be a better
        ...
        if (xPropInfo->hasPropertyByName(sTextPortionType))
        {
            ...
            else if ( sType.equals( sAnnotationEnd ) )
            {
                if (!bAutoStyles) {
                    ...
                    SvXMLElementExport aElem( GetExport(), true, XML_NAMESPACE_OFFICE, XML_ANNOTATION_END, sal_False, sal_False );
		...
		const Reference < XPropertySetInfo > & rPropSetInfo )
{
	...
	// If there still is no url, then the graphic is empty
	if( sURL.getLength() )
	{
...
{
	DBG_ASSERT( sal_False, "no API implementation available" );
}
...
{
    // check parameters and pre-conditions
