...
public:
    ...
    const Mapping & getCpp2Uno();
	const Mapping & getUno2Cpp();
	uno_Interface * mapToUno( const Any & rObj, typelib_InterfaceTypeDescription * pTo );
	...
	// XIdlReflection
	virtual Reference< XIdlClass > SAL_CALL forName( const OUString & rTypeName );
	virtual Reference< XIdlClass > SAL_CALL getType( const Any & rObj );
	...
	Reference< XIdlClass > forType( typelib_TypeDescription * pTypeDescr );
	Reference< XIdlClass > forType( typelib_TypeDescriptionReference * pRef );
};
...
public:
    ...
    virtual sal_Bool SAL_CALL equals( const Reference< XIdlClass >& xType );

    virtual sal_Bool SAL_CALL isAssignableFrom( const Reference< XIdlClass > & xType );
    virtual void SAL_CALL createObject( Any & rObj );

	// def impl ????
    virtual Sequence< Reference< XIdlClass > > SAL_CALL getClasses();
    virtual Reference< XIdlClass > SAL_CALL getClass( const OUString & rName );
    virtual Sequence< Reference< XIdlClass > > SAL_CALL getInterfaces();

	// structs, interfaces
    virtual Sequence< Reference< XIdlClass > > SAL_CALL getSuperclasses();
	// structs
    virtual Reference< XIdlField > SAL_CALL getField( const OUString & rName );
    virtual Sequence< Reference< XIdlField > > SAL_CALL getFields();
	// interfaces
    virtual Uik SAL_CALL getUik();
    virtual Reference< XIdlMethod > SAL_CALL getMethod( const OUString & rName );
    virtual Sequence< Reference< XIdlMethod > > SAL_CALL getMethods();
	// array
    virtual Reference< XIdlClass > SAL_CALL getComponentType();
    virtual Reference< XIdlArray > SAL_CALL getArray();
};
...
public:
	...
	// XIdlArray
    virtual void SAL_CALL realloc( Any & rArray, sal_Int32 nLen );
    virtual sal_Int32 SAL_CALL getLen( const Any & rArray );
    virtual Any SAL_CALL get( const Any & rArray, sal_Int32 nIndex );
