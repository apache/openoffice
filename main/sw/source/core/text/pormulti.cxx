...
void SwMultiPortion::CalcSize( SwTxtFormatter& rLine, SwTxtFormatInfo &rInf )
{
	...
	do
	{
		...
		{
			// An empty phonetic line doesn't need an ascent or a height.
			if( !pLay->Width() )
			{
				pLay->SetAscent( 0 );
				pLay->Height( 0 );
...
/*-----------------25.10.00 16:26-------------------
 * SwDoubleLinePortion::SetBrackets creates the bracket-structure
	...
	SwMultiPortion& rMulti, const SwMultiPortion* pEnvPor )
{
	...
	if ( rMulti.IsBidi() )
	{
		...
		// and the layout mode
		ASSERT( ! pEnvPor || pEnvPor->IsBidi(), "I expected a BidiPortion" )
	...
	SwMultiPortion& rMulti )
{
	...
	if( bRet )
	{
		...
		else if( rMulti.IsBidi() && ! pMulti )
		{
			...
			// the visual cursor traveling gets into trouble.
