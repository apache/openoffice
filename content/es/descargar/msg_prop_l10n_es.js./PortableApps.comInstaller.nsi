...
!define APPNAME "RapidSVN"	;<== Fill this
!define VERSION "0.9.6.0" ; <== Fill this
...
!define NAME "${APPNAME}, Portable Edition"
!define SHORTNAME "${APPNAME}"
!define FILENAME "${APPNAME}_Portable_${VERSION}"
!define CHECKRUNNING "${APPNAME}Portable.exe"
!define CLOSENAME "${APPNAME}, Portable Edition"
!define ADDONSDIRECTORYPRESERVE "App\${APPNAME}\plugins"
...
Section "!App Portable (required)"
	SetOutPath $INSTDIR
	RMDir /r "$INSTDIR\Other\${APPNAME}PortableSource"
	RMDir /r "$INSTDIR\Other\${APPNAME}Source"
	File /r "..\..\*.*"
	CreateDirectory "$INSTDIR\Data\settings"
	WriteINIStr "$INSTDIR\Data\settings\${APPNAME}PortableSettings.ini" "${APPNAME}PortableSettings" "AgreedToLicense" "YES"
