...
namespace drawinglayer
{
	namespace processor2d
	{
        ...
        bool VclPixelProcessor2D::tryDrawPolyPolygonColorPrimitive2DDirect(const drawinglayer::primitive2d::PolyPolygonColorPrimitive2D& rSource, double fTransparency)
        {
            ...
            const basegfx::BColor aPolygonColor(getBColorModifierStack().getModifiedColor(rSource.getBColor()));
        ...
        bool VclPixelProcessor2D::tryDrawPolygonHairlinePrimitive2DDirect(const drawinglayer::primitive2d::PolygonHairlinePrimitive2D& rSource, double fTransparency)
        {
            ...
            const basegfx::BColor aLineColor(getBColorModifierStack().getModifiedColor(rSource.getBColor()));

            getOutputDevice().SetFillColor();
            getOutputDevice().SetLineColor(Color(aLineColor));
            aLocalPolygon.transform(getCurrentTransformation());

            // try drawing; if it did not work, use standard fallback
            if(getOutputDevice().TryDrawPolyLineDirect(
                aLocalPolygon, 
                0.0, 
                fTransparency))
            {
        ...
        bool VclPixelProcessor2D::tryDrawPolygonStrokePrimitive2DDirect(const drawinglayer::primitive2d::PolygonStrokePrimitive2D& rSource, double fTransparency)
        {
            ...
            const basegfx::BColor aLineColor(
                getBColorModifierStack().getModifiedColor(
            ...
            if(basegfx::fTools::more(fLineWidth, 0.0))
            {
                ...
                aLineWidth = getCurrentTransformation() * aLineWidth;
			...
			switch(rCandidate.getPrimitive2DID())
			{
				...
				case PRIMITIVE2D_ID_POLYPOLYGONCOLORPRIMITIVE2D :
				{
                    ...
                    else
                    {
                        ...
                        const basegfx::BColor aPolygonColor(getBColorModifierStack().getModifiedColor(rPolyPolygonColorPrimitive2D.getBColor()));
