		...
		if (nVersion==DEFAULT_NUMBERING_CACHE_FORMAT_VERSION) //first version
		{
			*pIStm >> nNumIndex;
			...
			while (nNumIndex>=0 && nNumIndex<DEFAULT_NUM_VALUSET_COUNT) {
				...
				RelplaceNumRule(aNum,static_cast< sal_uInt16 >(nNumIndex),mLevel);
				*pIStm >> nNumIndex;
			}
			delete pIStm;
		}
	...
	if( pOStm ) {
		...
		*pOStm << nVersion;
		for(sal_Int32 nItem = 0; nItem < DEFAULT_NUM_VALUSET_COUNT;	nItem++ ) {
			if (IsCustomized(static_cast< sal_uInt16 >(nItem))) {
				...
				sal_uInt16 mLevel = 0x1;
				*pOStm << nItem;
				ApplyNumRule(aDefNumRule,static_cast< sal_uInt16 >(nItem),mLevel,false,true);
...
{
	String sRet;
	sal_uInt16 nLength(static_cast< sal_uInt16 >(aGrfDataLst.Count()));
